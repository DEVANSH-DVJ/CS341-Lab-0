...
public:
  /**
   * @brief Constructor
   * @param edges Edge list in the format { {{A, B}, C}, {{A, B}, C}, ...} where
   * A, B are node numbers and C is weight
   */
  ...
  void construct_spt() {
    while (!vheap->isEmpty()) {
      ...
      for (auto node : adj_list[minNode]) {
        if (vheap->find(node.first) &&
            minDist + node.second < vheap->getDistance(node.first)) {
...
int main() {
  Graph g = {{{0, 1}, 4}, {{0, 7}, 8}, {{1, 2}, 8}, {{1, 7}, 11}, {{2, 3}, 7},
             {{2, 8}, 2}, {{2, 5}, 4}, {{3, 4}, 8}, {{3, 5}, 14}, {{4, 5}, 10},
